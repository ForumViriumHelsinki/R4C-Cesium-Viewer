...
- **Compression**: Built-in compression (level 6) via `pg_dump --compress=6`
...
- **Format**: PostgreSQL custom format (created with `pg_dump --format=custom`, restored with `pg_restore`)
...
# Create dump in custom format with compression
...
1. **Manual Trigger Test**:
   ...
   pg_restore -l /tmp/regions4climate-YYYY-MM-DD.dump | head -20
   ```
