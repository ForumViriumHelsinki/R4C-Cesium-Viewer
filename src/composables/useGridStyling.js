...
export const indexToColorScheme = {
  ...
  combined_avgheatexposure: [
    { color: "gradient", range: "Combined Heat Exposure" },
  ],
};
...
export function useGridStyling() {
  ...
  const getColorForIndex = (indexValue, indexType) => {
    ...
    const colorString = colorEntry
      ? colorEntry.color
      : colorScheme[colorScheme.length - 1].color;
    return Cesium.Color.fromCssColorString(colorString).withAlpha(
      baseAlpha.value,
    );
  };
    ...
    for (const entity of dataSource.entities.values) {
      ...
      if (selectedIndex === "heat_index") {
        ...
        if (originalHeatIndex != null) {
          const reduction =
            mitigationStore.calculateTotalReductionForCell(entity);
          ...
          if (entity.polygon) {
            entity.polygon.material = getColorForIndex(
              newHeatIndex,
              "heat_index",
            );
          }
