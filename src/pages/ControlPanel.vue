  ...
  setup() {
    ...
    const hasAvailableAnalysis = computed(() => {
      ...
      if (currentLevel.value === 'postalCode' && currentView.value !== 'grid') {
        return (heatHistogramData.value && heatHistogramData.value.length > 0) || showSosEco.value || (currentView.value !== 'helsinki') || true;
      }
    ...
    // ## MODIFIED: 'cooling-centers' is removed from this config ##
    const analysisConfig = {
      'heat-histogram': { title: 'Heat Distribution Analysis', icon: 'mdi-chart-histogram', width: '800px', height: '600px' },
      'socioeconomics': { title: 'Socioeconomic Analysis', icon: 'mdi-account-group', width: '900px', height: '700px' },
      'landcover': { title: 'Land Cover Analysis', icon: 'mdi-leaf', width: '800px', height: '600px' },
      'scatter-plot': { title: 'Building Analysis', icon: 'mdi-chart-scatter-plot', width: '1000px', height: '700px' },
      'building-heat': { title: 'Building Heat Data', icon: 'mdi-thermometer', width: '800px', height: '600px' },
      'grid-options': { title: 'Statistical Grid Options', icon: 'mdi-grid', width: '600px', height: '500px' },
      'ndvi-analysis': { title: 'NDVI Vegetation Analysis', icon: 'mdi-leaf', width: '900px', height: '600px' }
    };

    const currentAnalysisTitle = computed(() => analysisConfig[currentAnalysis.value]?.title || '');
    const currentAnalysisIcon = computed(() => analysisConfig[currentAnalysis.value]?.icon || 'mdi-chart-line');
    const analysisDialogWidth = computed(() => analysisConfig[currentAnalysis.value]?.width || '800px');
    const analysisDialogHeight = computed(() => analysisConfig[currentAnalysis.value]?.height || '600px');
    ...
    const rotateCamera = () => { new Camera().rotateCamera(); };
