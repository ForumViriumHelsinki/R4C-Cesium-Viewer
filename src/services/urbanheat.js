      ...
      if (!inHelsinki) {
        if (features[i].properties.heat_timeseries) {
          ...
          const heatExposureValue = inHelsinki
            ...
            : heatTimeseriesValue?.find(({ date }) => date === targetDate)
                ?.avgheatexposure;
  ...
  async findUrbanHeatData(data) {
    ...
    try {
      ...
      for (let i = 0; i < data.features.length; i++) {
        ...
        await setAttributesFromApiToBuilding(
          feature.properties,
          urbanheat.features,
        );
      }
      console.log(
        "[UrbanHeat] ✅ Building heat attributes processing complete",
      );
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      if (properties.id == feature.properties.hki_id) {
        if (feature.properties.avgheatexposuretobuilding) {
          properties.avgheatexposuretobuilding =
            feature.properties.avgheatexposuretobuilding;
        }
        ...
        if (feature.properties.year_of_construction) {
          properties.year_of_construction =
            feature.properties.year_of_construction;
        }
        ...
        properties.c_julkisivu = decodingService.decodeFacade(
          properties.c_julkisivu,
        );
        properties.c_rakeaine = decodingService.decodeMaterial(
          properties.c_rakeaine,
        );
        properties.c_lammtapa = decodingService.decodeHeatingMethod(
          properties.c_lammtapa,
        );
        properties.c_poltaine = decodingService.decodeHeatingSource(
          properties.c_poltaine,
        );
...
const filterHeatTimeseries = (buildingProps) => {
  if (
    buildingProps.kavu &&
    typeof buildingProps.kavu === "number" &&
    buildingProps.kavu > 2015
  ) {
    const cutoffYear = buildingProps.kavu;
    buildingProps.heat_timeseries = buildingProps.heat_timeseries.filter(
      (entry) => {
