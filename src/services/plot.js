...
export default class Plot {
  ...
  showAllPlots() {
    switch (this.store.level) {
      case "postalCode":
        eventBus.emit(
          this.toggleStore.helsinkiView ? "showHelsinki" : "showCapitalRegion",
        );
        break;
      case "building":
  ...
  hideAllPlots() {
    switch (this.store.level) {
      case "postalCode":
        eventBus.emit(
          this.toggleStore.helsinkiView ? "hideHelsinki" : "hideCapitalRegion",
        );
        break;
      case "building":
 ...
 */
  toggleBearingSwitchesVisibility(status) {
    ...
    for (const direction of switchContainers) {
      const switchContainer = document.getElementById(
        `bearing${direction}SwitchContainer`,
      );
  ...
  updateTreeElements(status) {
    document.getElementById("nearbyTreeAreaContainer").style.visibility =
      status;
  ...
  initializePlotContainer(containerId) {
    ...
    container.style.visibility = this.toggleStore.showPlot
      ? "visible"
      : "hidden";
  }
