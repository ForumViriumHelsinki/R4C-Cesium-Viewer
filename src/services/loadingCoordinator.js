...
class LoadingCoordinator {
  ...
  get globalStore() {
    if (!this._globalStore) {
      try {
        ...
        console.warn(
          "Global store not available, using fallback:",
          error.message,
        );
        this._globalStore = {
          cesiumViewer: null,
  ...
  initializeSession(sessionId, layerConfigs, options) {
    ...
    console.log(
      `🚀 Starting loading session: ${sessionId} (${layerConfigs.length} layers)`,
    );
  }
  ...
  async loadBalanced(sessionId, layerConfigs) {
    ...
    if (criticalAndHigh.length > 0) {
      console.log(
        `⚡ Loading ${criticalAndHigh.length} critical/high priority layers`,
      );
      const criticalResults = await this.loadWithStaggering(
        criticalAndHigh,
        100,
      ); // 100ms stagger
    ...
    if (normalAndLow.length > 0) {
      console.log(
        `🔄 Loading ${normalAndLow.length} normal/low priority layers in parallel`,
      );
    ...
    if (priorityGroups.background?.length > 0) {
      console.log(
        `🌅 Scheduling ${priorityGroups.background.length} background layers`,
      );
  ...
  completeSession(sessionId, results) {
    ...
    this.performanceMetrics.avgLoadTime =
      ...
      this.performanceMetrics.layersLoaded;
    ...
    console.log(
      `✅ Session ${sessionId} completed in ${duration.toFixed(0)}ms`,
    );
  ...
  async intelligentPreload(context = {}) {
    ...
    if (preloadConfigs.length > 0) {
      console.log(
        `🔮 Starting intelligent preload of ${preloadConfigs.length} layers`,
      );
