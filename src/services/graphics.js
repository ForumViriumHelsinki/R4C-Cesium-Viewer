...
export default class Graphics {
  ...
  detectSupport() {
    ...
    if (!supportInfo.msaaSupported) {
      console.warn(
        "MSAA (Multi-Sample Anti-Aliasing) is not supported by this browser/hardware",
      );
    }
    if (!supportInfo.hdrSupported) {
      console.warn(
        "HDR (High Dynamic Range) rendering is not supported by this browser/hardware",
      );
    }
    if (!supportInfo.ambientOcclusionSupported) {
      console.warn(
        "Ambient Occlusion post-processing is not supported by this browser/hardware",
      );
    }
  }
  ...
  applyFxaaSettings() {
    ...
    if (fxaaStage) {
      ...
      console.log(
        `FXAA ${this.graphicsStore.fxaaEnabled ? "enabled" : "disabled"}`,
      );
    }
  }
  ...
  applyHdrSettings() {
    if (this.graphicsStore.hdrSupported) {
      ...
      console.log(
        `HDR ${this.graphicsStore.hdrEnabled ? "enabled" : "disabled"}`,
      );
  ...
  applyAmbientOcclusionSettings() {
    if (this.graphicsStore.ambientOcclusionSupported) {
      ...
      if (ambientOcclusion.enabled) {
        ...
        console.log(
          "Ambient Occlusion enabled with building-optimized settings",
        );
      } else {
  ...
  setupWatchers() {
    ...
    this.graphicsStore.$subscribe((mutation, state) => {
      if (
        mutation.events.some((e) =>
          ["msaaEnabled", "msaaSamples"].includes(e.key),
        )
      ) {
        this.applyMsaaSettings();
      }
