...
export default class Building {
  ...
  async createBuildingCharts(treeArea, avg_temp_c, buildingProps) {
    ...
    this.toggleStore.helsinkiView
      ? (this.propsStore.setBuildingHeatExposure(
          buildingProps._avgheatexposuretobuilding._value,
        ),
        eventBus.emit("newBuildingHeat"))
      : !this.toggleStore.helsinkiView
        ? buildingProps.heat_timeseries &&
          this.propsStore.setBuildingHeatTimeseries(
            buildingProps.heat_timeseries._value,
          )
        : this.propsStore.setGridBuildingProps(buildingProps);
  }
  ...
  async loadBuildings() {
    ...
    try {
      ...
      console.log(
        "[HelsinkiBuilding] ✅ Buildings processed and added to Cesium viewer",
      );
    } catch (error) {
  ...
  resetBuildingEntities() {
    ...
    for (
      let i = 0;
      i < buildingDataSource._entityCollection._entities._array.length;
      i++
    ) {
  ...
  filterBuildings(buildingsDataSource) {
    ...
    this.updateHeatHistogramDataAfterFilter(
      buildingsDataSource.entities._entities._array,
    );
  }
  ...
  updateHeatHistogramDataAfterFilter(entities) {
    ...
    const avg_temp = this.toggleStore.helsinkiView
      ? visibleEntities.map(
          (entity) => entity.properties._avgheatexposuretobuilding._value,
        )
      : visibleEntities
          .map((entity) => {
            const heatTimeseries =
              entity.properties.heat_timeseries?._value || [];
            const foundEntry = heatTimeseries.find(
              ({ date }) => date === targetDate,
            );
  ...
  soteBuildings(entity) {
    ...
    entity.show = this.toggleStore.helsinkiView
      ? kayttotark &&
        ([511, 131].includes(kayttotark) ||
          (kayttotark > 210 && kayttotark < 240))
  ...
  lowBuildings(entity) {
    entity._properties?.[
      this.toggleStore.helsinkiView ? "_i_kerrlkm" : "_kerrosten_lkm"
    ]?._value <= 6 && (entity.show = false);
  }
  /**
  ...
  showAllBuildings(buildingsDataSource) {
    ...
    for (
      let i = 0;
      i < buildingsDataSource._entityCollection._entities._array.length;
      i++
    ) {
      // Show the entity
  ...
  outlineByTemperature(entity, property, values) {
    ...
    const foundEntry = heatTimeseries.find(
      ({ date }) => date === this.store.heatDataDate,
    );

    const shouldOutlineYellow = !this.toggleStore.helsinkiView
      ? foundEntry && values.includes(foundEntry.avg_temp_c)
      : entity._properties[property] &&
        values.includes(entity._properties[property]._value);
  ...
  highlightBuildingInViewer(id) {
    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      this.outlineById(
        entity,
        this.toggleStore.helsinkiView ? "id" : "kiitun",
        id,
      );
    }
  }
...
const filterHeatTimeseries = (buildingProps) => {
  if (buildingProps._kavu && typeof buildingProps._kavu._value === "number") {
    ...
    buildingProps._heat_timeseries._value =
      buildingProps._heat_timeseries._value.filter((entry) => {
