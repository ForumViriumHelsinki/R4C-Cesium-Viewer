...
export default class Othernature {
  ...
  async loadOtherNature() {
    try {
      ...
      console.log(
        `✓ Other nature data loaded for postal code ${this.store.postalcode}`,
      );
      return data;
    } catch (error) {
  ...
  async addOtherNatureDataSource(data, metadata = {}) {
    try {
      ...
      if (!metadata.fromCache) {
        ...
        console.log(
          `✓ Restored ${entities.length} other nature entities from cache`,
        );
      }
    } catch (error) {
  ...
  setOtherNaturePolygonMaterialColor(entity, category) {
    switch (category) {
      case "310":
        entity._polygon._material._color._value =
          Cesium.Color.LIGHTGREY.withAlpha(0.5);
        break;
      case "410":
        entity._polygon._material._color._value =
          Cesium.Color.SANDYBROWN.withAlpha(0.5);
        break;
      case "130":
        entity._polygon._material._color._value =
          Cesium.Color.ROSYBROWN.withAlpha(0.5);
        break;
    }
  }
}
