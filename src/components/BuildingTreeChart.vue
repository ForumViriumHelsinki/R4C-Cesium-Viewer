...
export default {
	setup() {
		...
		const createBuildingTreeBarChart = () => {
			...
			const svg = plotService.createSVGElement( margin, width, height, '#buildingTreeChartContainer' );

			const xScale = plotService.createScaleBand( [ address, postinumero ], width );
			const yScale = plotService.createScaleLinear( 0, Math.max( treeArea, store.averageTreeArea ), [ height, 0 ] );
