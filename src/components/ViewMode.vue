	...
	<div class="view-mode-selector">
		<div class="view-mode-cards">
			...
			>
				<div class="card-header">
					...
					<v-icon v-if="activeViewMode === 'capitalRegionView'" class="check-icon" color="primary">
						mdi-check-circle
					</v-icon>
				</div>

				<p class="mode-description">
					Explore urban heat patterns across the entire Helsinki metropolitan area (~200 postal
					codes)
				</p>

				<div class="mode-features">
					<v-chip size="x-small" class="feature-chip">
						<v-icon start size="12"> mdi-thermometer </v-icon>
						Heat Analysis
					</v-chip>
					<v-chip size="x-small" class="feature-chip">
						<v-icon start size="12"> mdi-home-group </v-icon>
						Building Data
					</v-chip>
					<v-chip size="x-small" class="feature-chip">
						<v-icon start size="12"> mdi-leaf </v-icon>
						Green Coverage
					</v-chip>
				</div>
			</div>
			...
			>
				<div class="card-header">
					<v-icon class="mode-icon" :color="activeViewMode === 'gridView' ? 'primary' : 'grey'">
						mdi-grid
					</v-icon>
					<h4 class="mode-title">Statistical Grid</h4>
					<v-icon v-if="activeViewMode === 'gridView'" class="check-icon" color="primary">
						mdi-check-circle
					</v-icon>
				</div>

				<p class="mode-description">
					Analyze data using standardized 250m × 250m statistical grid cells for precise spatial
					analysis
				</p>
			...
			>
				<div class="card-header">
					<v-icon class="mode-icon" :color="activeViewMode === 'helsinkiHeat' ? 'primary' : 'grey'">
						mdi-city-variant
					</v-icon>
					<h4 class="mode-title">Helsinki Focus</h4>
					<v-icon v-if="activeViewMode === 'helsinkiHeat'" class="check-icon" color="primary">
						mdi-check-circle
					</v-icon>
				</div>

				<p class="mode-description">
					Detailed analysis focused on Helsinki city with enhanced vegetation and social
					infrastructure data
				</p>

				<div class="mode-features">
					<v-chip size="x-small" class="feature-chip">
						<v-icon start size="12"> mdi-tree </v-icon>
						Vegetation
					</v-chip>
					<v-chip size="x-small" class="feature-chip">
		...
		<div class="current-view-info">
			<div class="info-section">
				<v-icon class="info-icon" size="16" color="primary"> mdi-information </v-icon>
				<span class="info-text">{{ currentViewInfo }}</span>
			</div>

			<div class="coverage-info">
				<v-chip size="small" :color="getViewModeColor()" variant="tonal">
					{{ getCoverageText() }}
				</v-chip>
			</div>
		</div>
