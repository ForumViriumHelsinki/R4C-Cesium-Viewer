...
export default {
	setup() {
			...
			svg
				...
				.on('mouseover', (event, d) =>
					plotService.handleMouseover(
						...
						(
							data
						) => `${propsStore.numericalSelect.text}: ${data.xData.toFixed(2)}<br>Temperature: ${data.yData.toFixed(2)}°C<br>${propsStore.categoricalSelect.text}: ${data.name}
            <br>Left click highlights the building on map`
					)
				)
				.on('mouseout', () => plotService.handleMouseout(tooltip))
				.on('click', (_event, d) => buildingService.highlightBuildingInViewer(d.buildingId))
		}
		...
		const addDataForHSYScatterPlot = (urbanHeatDataAndMaterial, entity) => {
			...
			if (
				props?.[propsStore.categoricalSelect.value]?._value &&
				props?.[propsStore.numericalSelect.value]?._value
			) {
				...
				if (props?._area_m2?._value > 225 && numericalValue < 99999999 && numericalValue !== 0) {
					const heatValue = props?._heat_timeseries?._value.find(
		...
		const createLegend = (svg, width, _margin, values, labelsWithAverage, colorScale) => {
			const legend = svg
				.append('g')
				.attr('class', 'legend')
		...
		const createUniqueValuesList = (features) => {
			const uniqueValues = new Set(
				features.map((feature) => feature[propsStore.categoricalSelect.text])
			)
