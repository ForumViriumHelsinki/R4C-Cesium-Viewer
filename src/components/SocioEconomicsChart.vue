  ...
  setup() {
    ...
    const createBars = (svg, data, xScale, yScale, height, tooltip, xOffset, barColor, name) => {
      ...
      bar.append('rect')
        ...
        .on('mouseover', (event, d) =>
          plotService.handleMouseover(tooltip, 'socioeonomicsContainer', event, d, (data) => 
            `Area: ${name}<br>Indicator: ${data.label}<br>Value: ${data.value}`))
    ...
    const createSocioEconomicsDiagram = (sosData, statsData) => {
      if (sosData) {
        ...
        plotService.addTitle(svg, `Compare vulnerability in ${globalStore.nameOfZone} to:`, width / 2, margin);
      }
    };
