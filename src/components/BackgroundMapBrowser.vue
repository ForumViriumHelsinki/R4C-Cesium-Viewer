...
export default {
  ...
  setup() {
    ...
    const hasSelection = computed(() => {
      return (selectedBasicMap.value && selectedBasicMap.value !== 'default') ||
    ...
    const currentSelectionText = computed(() => {
      if (selectedFloodLayer.value && selectedFloodLayer.value !== 'none') {
        ...
        if (floodMap.startsWith('Hulevesitulva')) {
          return floodMap.includes('52mm') ? 'Stormwater Flood: 52mm/hour' : 'Stormwater Flood: 80mm/hour';
      ...
      try {
        ...
        const extractLayers = (groups) => {
          ...
          groups.forEach(group => {
            if (group.layers) {
              layers.push(...group.layers.map(layer => ({
                id: layer.id,
                name: layer.name,
