...
export default {
  ...
  setup(props) {
    ...
    const drawChart = () => {
      ...
      const width = 400, height = 250, margin = { top: 20, right: 30, bottom: 50, left: 50 };
      const xScale = d3.scaleBand().domain(labels).range([margin.left, width - margin.right]).padding(0.2);
      const yScale = d3.scaleLinear().domain([0, Math.max(...bins)]).nice().range([height - margin.bottom, margin.top]);
