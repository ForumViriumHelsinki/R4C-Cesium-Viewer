...
export const useMitigationStore = defineStore("mitigation", {
  state: () => ({
    ...
    effecting5GridCellsCoolingAreaMax:
      Math.PI * (Math.pow(125, 2) + Math.pow(125, 2)), // Area = πr²  98175.77042 cooling area, 12271.8463025 park area, 2337 count
  ...
  actions: {
    async setGridCells(datasource) {
      this.gridCells = datasource.entities.values
        .filter(
          (entity) =>
            entity.properties?.final_avg_conditional?.getValue() != null,
        )
        .map((entity) => {
    ...
    calculateCoolingCenterEffect(entity) {
      ...
      for (const center of this.coolingCenters) {
        const distance = Math.sqrt(
          Math.pow(center.euref_x - euref_x, 2) +
            Math.pow(center.euref_y - euref_y, 2),
        );
      ...
      return (
        this.gridCells
          ...
          .map((neighbor) => ({
            ...neighbor,
            distance: Math.sqrt(
              Math.pow(sourceCell.x - neighbor.x, 2) +
                Math.pow(sourceCell.y - neighbor.y, 2),
            ),
          }))
    ...
    calculateParksEffect(sourceEntity, totalAreaConverted) {
      ...
      const sourceReduction =
        (totalAreaConverted / this.gridArea) * this.parks2022CoolingConstant;
      ...
      const areaOfInfluence =
        totalAreaConverted * this.getParksEffect(totalAreaConverted);
      ...
      neighbors.forEach((neighbor, index) => {
        ...
        if (index < 4) {
          if (areaOfInfluence >= this.effecting5GridCellsCoolingAreaMax)
            ...
            const range =
              this.effecting5GridCellsCoolingAreaMax -
              this.effecting1GridCellsCoolingAreaMax;
            const progress =
              areaOfInfluence - this.effecting1GridCellsCoolingAreaMax;
            neighborReduction =
              this.parks2022CoolingConstant * ((progress / range) * 0.5);
          }
        } else {
          if (areaOfInfluence >= this.effecting9GridCellsCoolingAreaMax)
            ...
            const range =
              this.effecting9GridCellsCoolingAreaMax -
              this.effecting5GridCellsCoolingAreaMax;
            const progress =
              areaOfInfluence - this.effecting5GridCellsCoolingAreaMax;
            neighborReduction =
              this.parks2022CoolingConstant * ((progress / range) * 0.25);
          }
        }
        if (neighborReduction > 0) {
          const neighborOriginalIndex = this.modifiedHeatIndices[neighbor.id];
          const neighborNewIndex = Math.max(
            0,
            neighborOriginalIndex - neighborReduction,
          );
    ...
    preCalculateGridImpacts() {
      ...
      this.gridCells.forEach((cell) => {
        ...
        this.gridCells.forEach((otherCell) => {
          ...
          if (
            Math.abs(cell.x - otherCell.x) <= 1000 &&
            Math.abs(cell.y - otherCell.y) <= 1000
          ) {
            const distance = Math.sqrt(
    ...
    getCoolingCenterCount(gridId) {
      return this.coolingCenters.filter((center) => center.grid_id === gridId)
        .length;
    },
    ...
    getReductionValue(distance) {
      return distance > this.reachability
        ? 0
        : this.maxReduction -
            (distance / this.reachability) *
              (this.maxReduction - this.minReduction);
    },
