...
export const useLoadingStore = defineStore("loading", {
  ...
  getters: {
    ...
    overallProgress: (state) => {
      const activeLoading = Object.entries(state.layerLoading).filter(
        ([_, loading]) => loading,
      );
  ...
  actions: {
    ...
    startLayerLoading(layerName, options = {}) {
      ...
      this.loadingMessages[layerName] =
        options.message || `Loading ${layerName}...`;
    ...
    completeLayerLoading(layerName, success = true) {
      ...
      if (success) {
        ...
        this.loadingProgress[layerName].current =
          this.loadingProgress[layerName].total;
      ...
      if (this.loadingTimes[layerName]) {
        this.loadingTimes[layerName].duration =
          Date.now() - this.loadingTimes[layerName].startTime;
      }
    ...
    async startLayerLoadingWithCache(layerName, options = {}) {
      // Check cache first
      const cacheKey =
        options.cacheKey || `${layerName}-${options.postalCode || "global"}`;
    ...
    async completeLayerLoadingWithCache(
      layerName,
      success = true,
      data = null,
      options = {},
