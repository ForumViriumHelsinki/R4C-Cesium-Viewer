 ...
 */
async function loadTravelTimeData( from_id ) {

	const urbanheatdata = fetch( "https://geo.fvh.fi/r4c/collections/hki_travel_time/items?f=json&limit=2&from_id=" + from_id )
...
function addTravelTimeLabels(traveldata, from_id) {
    ...
    viewer.dataSources.add(dataSourceTravelLabel);

    const dataSource = viewer.dataSources.getByName('TravelTimeGrid');
    ...
    addTravelLabelDataSource( geoJsonData );

}

function removeTravelTimeGridAndAddDataGrid( ) {
 ...
 * 
 */
function addTravelLabelDataSource( data ) {
