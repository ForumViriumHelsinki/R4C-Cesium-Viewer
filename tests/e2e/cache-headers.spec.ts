...
test.describe('Cache Header Verification', () => {
	...
	test(
		...
		async ({ request }) => {
			...
			const assetMatches = html.matchAll(/\/assets\/[\w-]+\.[a-f0-9]{8}\.\d+\.\d+\.\d+\.(js|css)/g);
	...
	test(
		...
		async ({ request }) => {
			...
			const hashedAssetPattern = /\/assets\/[\w-]+\.[a-f0-9]{8}\.\d+\.\d+\.\d+\.(js|css)/;
	...
	test(
		...
		async ({ request }) => {
			...
			const cssMatches = html.matchAll(/\/assets\/[\w-]+\.[a-f0-9]{8}\.\d+\.\d+\.\d+\.css/g);
