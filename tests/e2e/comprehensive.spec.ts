...
test.describe("R4C Climate Visualization Comprehensive Tests", () => {
  ...
  test.describe("Basic Application Functionality", () => {
    test("should load main page with correct title and elements", async ({
      page,
    }) => {
  ...
  test.describe("Building Information and Properties", () => {
    test("should display building information when building is selected", async ({
      page,
    }) => {
    ...
    test("should handle building data for different locations", async ({
      page,
    }) => {
      ...
      for (const location of buildingTestLocations) {
        ...
        try {
          ...
          if (await buildingButton.isVisible({ timeout: 1000 })) {
            ...
            console.log(
              `Building info available at ${location.name}: ${hasInfo}`,
            );
          }
        } catch (error) {
  ...
  test.describe("Statistical Grid and Data Layers", () => {
    test("should enable and interact with statistical grid", async ({
      page,
    }) => {
      try {
  ...
  test.describe("Data Visualization Features", () => {
    ...
    test("should support timeline functionality if available", async ({
      page,
    }) => {
