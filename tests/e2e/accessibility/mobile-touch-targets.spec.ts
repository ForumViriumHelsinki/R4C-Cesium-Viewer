...
cesiumDescribe("Mobile Touch Targets @accessibility @mobile", () => {
  ...
  cesiumTest.describe("Navigation Buttons - Mobile", () => {
    cesiumTest("should meet 44px minimum touch target on mobile (375px)", async ({ cesiumPage }) => {
      // iPhone SE viewport
    ...
    cesiumTest("should maintain 44px minimum on very small mobile (320px)", async ({ cesiumPage }) => {
  ...
  cesiumTest.describe("Touch Action Optimization", () => {
    cesiumTest("should have touch-action: manipulation on interactive elements", async ({ cesiumPage }) => {
      await cesiumPage.setViewportSize({ width: 375, height: 667 });

      // Check navigation buttons have touch-action set
      const signOutButton = cesiumPage.getByLabel("Sign out");
      const touchAction = await signOutButton.evaluate((el) =>
