...
cesiumDescribe('Building Filters Accessibility', () => {
  ...
  cesiumTest.describe('Universal Building Filters', () => {
    cesiumTest('should display "Only tall buildings" filter in all contexts', async ({ cesiumPage }) => {
      // Should be visible in default view
      await expect(cesiumPage.getByText('Only tall buildings')).toBeVisible();
      
      const tallBuildingsToggle = cesiumPage.getByText('Only tall buildings').locator('..').locator('input[type="checkbox"]');
    ...
    cesiumTest('should display Filters section header consistently', async ({ cesiumPage }) => {
      await expect(cesiumPage.getByText('Filters', { exact: true })).toBeVisible();
    ...
    cesiumTest('should maintain tall buildings filter state across contexts', async ({ cesiumPage }) => {
      const tallBuildingsToggle = cesiumPage.getByText('Only tall buildings').locator('..').locator('input[type="checkbox"]');
  ...
  cesiumTest.describe('Context-Adaptive Building Filters', () => {
    cesiumTest('should show "Only public buildings" in Capital Region view', async ({ cesiumPage }) => {
      await helpers.navigateToView('capitalRegionView');
      
      // Should show "Only public buildings" label
      await expect(cesiumPage.getByText('Only public buildings')).toBeVisible();
      
      const publicBuildingsToggle = cesiumPage.getByText('Only public buildings').locator('..').locator('input[type="checkbox"]');
    ...
    cesiumTest('should show "Only public buildings" in Grid view', async ({ cesiumPage }) => {
      await helpers.navigateToView('gridView');
    ...
    cesiumTest('should change label to "Only social & healthcare buildings" in Helsinki view', async ({ cesiumPage }) => {
      ...
      const hasPublicLabel = await cesiumPage.getByText('Only public buildings').isVisible();
      const hasSocialLabel = await cesiumPage.getByText('Only social &').isVisible();
  ...
  cesiumTest.describe('Helsinki-Specific Building Filters', () => {
    cesiumTest('should show "Built before summer 2018" only in Helsinki view', async ({ cesiumPage }) => {
      // In default Capital Region view, this filter should not be visible
      await helpers.navigateToView('capitalRegionView');
      await expect(cesiumPage.getByText('Built before summer 2018')).not.toBeVisible();
    ...
    cesiumTest('should handle Helsinki filter when it becomes available', async ({ cesiumPage }) => {
  ...
  cesiumTest.describe('Building Filter Accessibility', () => {
    cesiumTest('should have consistent styling for all filter toggles', async ({ cesiumPage }) => {
      ...
      const filterToggles = cesiumPage.locator('.switch-container').filter({ has: cesiumPage.locator('input[type="checkbox"]') });
    ...
    cesiumTest('should provide visual feedback for filter state changes', async ({ cesiumPage }) => {
      const tallBuildingsToggle = cesiumPage.getByText('Only tall buildings').locator('..').locator('input[type="checkbox"]');
      const slider = cesiumPage.getByText('Only tall buildings').locator('..').locator('.slider');
  ...
  cesiumTest.describe('Building Filter Responsiveness', () => {
    cesiumTest('should maintain filter functionality across different viewports', async ({ cesiumPage }) => {
      const viewports = [
