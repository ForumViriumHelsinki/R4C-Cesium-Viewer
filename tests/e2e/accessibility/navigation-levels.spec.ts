...
cesiumDescribe('Navigation Levels Accessibility', () => {
  ...
  cesiumTest.describe('Start Level (Initial State)', () => {
    cesiumTest('should display start level interface correctly', async ({ cesiumPage }) => {
      ...
      const cameraButton = cesiumPage.getByRole('button').filter({ has: cesiumPage.locator('.mdi-compass') });
  ...
  cesiumTest.describe('Postal Code Level Navigation', () => {
    cesiumTest('should transition to postal code level on map interaction', async ({ cesiumPage }) => {
      // Click on map to select postal code area
      await helpers.drillToLevel('postalCode');
      
      // Wait for level transition by checking for postal code specific UI elements
      await cesiumPage.waitForSelector('text="Building Scatter Plot", text="Area properties"', { timeout: 10000 }).catch(() => {});
    ...
    cesiumTest('should maintain view mode selection at postal code level', async ({ cesiumPage }) => {
  ...
  cesiumTest.describe('Building Level Navigation', () => {
    cesiumTest('should transition to building level from postal code', async ({ cesiumPage }) => {
      ...
      await cesiumPage.waitForSelector('text="Building heat data", text="Building properties"', { timeout: 10000 }).catch(() => {});
    ...
    cesiumTest('should handle back navigation from building to postal code', async ({ cesiumPage }) => {
    ...
    cesiumTest('should display building heat data variants based on view', async ({ cesiumPage }) => {
      const testViews = [
  ...
  cesiumTest.describe('Level-Specific Feature Access', () => {
    cesiumTest('should enable trees layer only with postal code selection', async ({ cesiumPage }) => {
      ...
      const currentView = await cesiumPage.locator('input[value="capitalRegionView"]').isChecked() ? 'capitalRegion' : 'grid';
    ...
    cesiumTest('should handle timeline interactions at postal code and building levels', async ({ cesiumPage }) => {
      // Navigate to postal code level
      await helpers.drillToLevel('postalCode');
      // Wait for postal code UI
      await cesiumPage.waitForSelector('text="Building Scatter Plot"', { timeout: 10000 }).catch(() => {});
  ...
  cesiumTest.describe('Accessibility Compliance for Navigation', () => {
    ...
    cesiumTest('should provide clear navigation state indicators', async ({ cesiumPage }) => {
      ...
      await cesiumPage.waitForSelector('[role="tooltip"], .v-tooltip', { timeout: 2000 }).catch(() => {});
