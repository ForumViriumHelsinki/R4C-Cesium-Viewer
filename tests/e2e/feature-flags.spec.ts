...
test.describe('Feature Flags Panel', () => {
	...
	test('Feature Flags button is visible in navigation', async ({ page }) => {
		...
		const featureFlagsButton = page.getByRole('button', { name: /Feature Flags/i });
	...
	test('Toggle a feature flag and verify persistence', async ({ page, context }) => {
		...
		await expect(ndviSwitchAfterReload).toHaveAttribute('aria-checked', initialState ? 'false' : 'true');
	});
	...
	test('Reset all flags confirmation dialog', async ({ page }) => {
		...
		const resetAllButton = page.getByRole('button', { name: /Reset All to Defaults/i });
	...
	test('Feature flags panel shows enabled count', async ({ page }) => {
		...
		const enabledCountChip = page.locator('.v-chip').filter({ hasText: /\d+\s*\/\s*\d+\s*enabled/i });
