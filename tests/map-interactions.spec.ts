...
test.describe('Map Interactions and Navigation', () => {
  ...
  test('should handle building selection workflow', async ({ page }) => {
    ...
    const buildingInfo = page.locator('[data-testid="building-information"], .building-information, .tooltip');
    if (await buildingInfo.count() > 0) {
      await expect(buildingInfo.first()).toBeVisible();
    }
    
    // Check for building properties button
    const buildingPropsButton = page.getByRole('button', { name: /building.*properties/i });
  ...
  test('should handle camera controls', async ({ page }) => {
    ...
    const viewToggle = page.locator('input[type="checkbox"]').filter({ hasText: /2D.*view|3D.*view/i });
  ...
  test('should handle timeline controls', async ({ page }) => {
    ...
    const timeline = page.locator('[data-testid="timeline"], .timeline, .heat-slider');
    
    if (await timeline.count() > 0) {
      await expect(timeline.first()).toBeVisible();
      
      // Look for date/time controls
      const timeControls = page.locator('input[type="range"], .v-slider, select');
  ...
  test('should handle return navigation', async ({ page }) => {
    ...
    const returnButton = page.getByRole('button', { name: /return|back|arrow.*left/i });
