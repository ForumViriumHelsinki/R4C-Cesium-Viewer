  ...
  test("should handle layer loading smoothly", async ({ page }) => {
    ...
    if ((await vegToggle.count()) > 0) {
      ...
      await page
        ...
        .catch((e) =>
          console.warn("Layer load network idle timeout:", e.message),
        );
      ...
      await page
        ...
        .catch((e) =>
          console.warn("DOM load after uncheck timeout:", e.message),
        );
    }
  });
    ...
    if ((await progressIndicators.count()) > 0) {
      ...
      if (await progress.isVisible()) {
        ...
        await page
          ...
          .catch((e) =>
            console.warn("Progress completion timeout:", e.message),
          );
      }
    }
  ...
  test("should handle memory efficiently during long session", async ({
    ...
    for (const position of interactions) {
      ...
      await page
        ...
        .catch((e) =>
          console.warn(
            `Position ${position.x},${position.y} load timeout:`,
            e.message,
          ),
        );
...
test.describe("Bundle Size and Dynamic Import Performance", () => {
  ...
  test("should load Cesium as separate chunk (dynamic import)", async ({
    ...
    // Wait for both Cesium chunk AND network idle to ensure all resources loaded
    await Promise.all([
