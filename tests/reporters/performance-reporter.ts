...
class PerformanceReporter implements Reporter {
  ...
  constructor(options: { baselineFile?: string } = {}) {
    const baselineFile =
      ...
      path.join(process.cwd(), "tests/performance-baselines.json");
    ...
    if (fs.existsSync(baselineFile)) {
      try {
        ...
        console.log(
          `[Performance Reporter] Loaded baselines from ${baselineFile}`,
        );
      } catch (error) {
        console.warn(
          `[Performance Reporter] Failed to load baselines: ${error}`,
        );
      }
    } else {
      console.log(
  ...
  async onEnd(result: FullResult) {
    ...
    const allTests = Array.from(this.suiteMetrics.values()).flatMap(
      (s) => s.tests,
    );
    const avgDuration =
      allTests.length > 0
