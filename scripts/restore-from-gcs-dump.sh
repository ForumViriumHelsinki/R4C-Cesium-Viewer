...
if [ -n "$LOCAL_FILE" ]; then
  ...
  echo -e "${GREEN}✅ Downloaded ${DUMP_SIZE} (native PostgreSQL custom format with built-in compression)${NC}"
fi
...
echo ""
echo -e "${BLUE}📥 Restoring database with pg_restore...${NC}"
echo -e "${YELLOW}⏱️  This may take several minutes...${NC}"

# Detect CPU cores for parallel restore
...
echo -e "${BLUE}🚀 Using $PARALLEL_JOBS parallel jobs for faster restore${NC}"

# Use pg_restore with parallel jobs
# -j: number of parallel jobs
# -v: verbose output
# --no-owner: skip ownership (we're restoring as postgres)
